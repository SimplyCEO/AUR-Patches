...
PKGNAME=""
get_package_name() { local FILETMP=$(mktemp); local PACKAGE=""; local LCOUNT=0; local KEY="$1"; while IFS= read -r LINE; do if printf "${LINE}" | grep -q "^${KEY}="; then PACKAGE="${LINE#${KEY}=}"; if [ "${PACKAGE}" = "(" ]; then pkgarray $LCOUNT "PKGBUILD" "${KEY}"; PACKAGE="${PKGARRAY}"; fi; printf "PATCHER_RETURN=${PACKAGE}\n" >> "${FILETMP}"; break; fi; LCOUNT=$((LCOUNT + 1)); if [ $LCOUNT -gt 20 ]; then break; fi; printf "${LINE}\n" >> "${FILETMP}"; done < PKGBUILD; . "${FILETMP}"; PACKAGE="${PATCHER_RETURN}"; rm "${FILETMP}"; if [ -z "${PACKAGE}" ]; then return 1; fi; PKGNAME="${PACKAGE}"; return 0; }

# --> Retrieve package file from repository, if exist.
fetch_patch() { curl -s -o PKGBUILD.patch "https://raw.githubusercontent.com/SimplyCEO/AUR-Patches/refs/heads/master/${PKGNAME}/PKGBUILD.patch";  if [ $? -ne 0 ] || grep -iq "NOT FOUND" PKGBUILD.patch || grep -iq "MOVED PERMANENTLY" PKGBUILD.patch; then rm -f PKGBUILD.patch; return 1; fi; return 0; }
